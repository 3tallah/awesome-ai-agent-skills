...
This agent can also use workspace prompts based on the awesome-design-md collection when a user asks for a site-inspired visual direction such as Vercel, Stripe, Linear, Notion, Apple, or Airbnb.

### Prompt to Use

- `.github/prompts/design-md-workflow.prompt.md`

### When to Use It

- The user asks to match or emulate a public website's visual system
- The user wants a component, page, or section in the style of a known product brand
- The user asks for color, typography, spacing, or component guidance based on a named site
- The user wants design-system comparison between two public sites

### How to Apply It

- Fetch the target `DESIGN.md` from the awesome-design-md repository using the raw GitHub URL pattern documented in the prompt
- Treat the fetched `DESIGN.md` as the visual source of truth for color, typography, spacing, elevation, and responsive behavior
- Map those values into existing AEM, HTL, Tailwind, and design-token conventions instead of copying CSS literally
- Prefer existing project tokens and Tailwind utilities first; add new variables only when the current system has a real gap
- Do not copy logos, proprietary artwork, or branded content. Use the design language, not brand assets
- Preserve accessibility, authoring usability, and component reusability while applying the style system
